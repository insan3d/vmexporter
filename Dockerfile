...
RUN python3 -m venv --without-pip /opt/venv \
 && python3 -m pip install --no-cache-dir --progress-bar=off --root-user-action=ignore --target "$(find /opt/venv -name site-packages)" -r /tmp/requirements.txt \
 \
...
FROM base as vmexporter

COPY ./vmexporter.py /opt/vmexporter.py
